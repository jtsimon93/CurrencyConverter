...
class MainForm(npyscreen.ActionForm):
    def create(self):
        ...
        self.convert_from = self.add(npyscreen.TitleCombo, name="Convert From: ", values=self.getOptions(),
                                     begin_entry_at=25)
        self.convert_to = self.add(npyscreen.TitleCombo, name="Convert To: ", values=self.getOptions(),
                                   begin_entry_at=25)
    ...
    def show_results(self):
        ...
        try:
            float(self.amount_to_convert.value)
        except ValueError:
            npyscreen.notify_confirm('You did not enter a valid amount to convert. Please try again.', title='Input Error')
            return
